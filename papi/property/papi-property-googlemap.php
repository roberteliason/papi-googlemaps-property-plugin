...
class Papi_Property_GoogleMap extends Papi_Property {
	...
	public function html() {
		...
		<div id="<?php echo md5( $this->get_slug() ) ?>" class="map-canvas"
		     data-slug="<?php echo md5( $this->get_slug() ) ?>" data-lat="<?php echo $lat ?>"
	 ...
	 * Setup actions.
	 *
	 * @todo send up a flag if the API Key has not been set!
	 */
	protected function setup_actions() {
		$options        = $this->get_options();
		$plugin_api_key = API::API_Key();
		$api_key        = isset( $options->settings->api_key ) ? $options->settings->api_key : $plugin_api_key;

		wp_enqueue_script( 'google_maps', "https://maps.googleapis.com/maps/api/js?key=" . $api_key . "&signed_in=false", 'jquery', false, true );
		wp_enqueue_script( 'papi_property_googlemap', PAPI_GM_PROPERTY_BASEURL . 'resources/js/papi-property-googlemap.js', [
			'jquery',
			'google_maps'
		], false, true );
	}
